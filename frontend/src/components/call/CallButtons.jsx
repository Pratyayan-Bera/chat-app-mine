...
export default function CallButtons({ userId, userName }) {
  ...
  // Disable buttons only when in the middle of a call
  const isDisabled = ['calling', 'connecting', 'active'].includes(callState);

  return (
    <div className="flex items-center space-x-2">
      {/* Debug info */}
      <span className="text-xs text-gray-500 italic">state: {callState}</span>
